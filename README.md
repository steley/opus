...
### 发布
- 「发布」→ 确认框（三组、组分隔线）：**阅后即焚**（可选）、**有效期**（默认 30 天，可选 1 小时/12 小时/24 小时/1 天/15 天/30 天/90 天/180 天/365 天，到期自动删除并在阅读页提醒）、**查看密码**（可选 ≥4 位）、**管理密码**（必填 ≥8 位，两次确认；拥有编辑+删除权限），密码支持大小写字母/数字/特殊字符
